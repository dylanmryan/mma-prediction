# Experiments — the full record

Everything this project measured, kept out of the README so the front door
stays readable. Nothing here is a summary: these are the original write-ups,
with the numbers, the losers, and the multiple-comparisons arithmetic
attached.

The short version, and the reason it is worth reading: **one feature block out
of eight ever cleared its bar on its own.** Almost every experiment below is a
negative result, recorded because a negative measured against a
pre-registered bar is worth more than an unmeasured hope — and because the
project's credibility rests on the ones that failed being here too.

Pre-registrations live in `docs/superpowers/plans/`; the decision artifacts
they produced are in `models/walkforward/`.
